...
const Register = () => {
  ...
  const { initOAuth, state: oauthState } = useLoginWithOAuth({
    onComplete: ({ user, isNewUser }) => {
      console.log('OAuth sign up successful:', { user, isNewUser });
    },
    onError: error => {
  ...
  const handleEmailSignUp = async () => {
    try {
      // This will open Privy's modal with email sign up
      console.log('Opening Privy email sign up modal...');
      await login();
    } catch (error) {
  ...
  const handleGoogleSignUp = async () => {
    try {
      // Use Privy's OAuth hook for Google sign up
